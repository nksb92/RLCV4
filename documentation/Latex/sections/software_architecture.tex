...
\begin{figure}[H]
  ...
  \begin{circuitikz}[>=latex]
    % Core 0 block
    \draw[dashed, thick, fill=gray!5, rounded corners] (0.0, 0.0) rectangle (5.4, 5.4);
    ...
    % Core 1 block
    \draw[dashed, thick, fill=gray!5, rounded corners] (10.8, 0.0) rectangle (16.2, 5.4);
...
\begin{figure}[H]
  ...
  \begin{circuitikz}[>=latex]
    % Timer Manager block
    \draw[dashed, thick, fill=gray!5, rounded corners] (2.8, 0.5) rectangle (8.6, 3.5);
...
\begin{figure}[H]
  ...
  \begin{circuitikz}[>=latex]
    ...
    \node[draw, dashed, fill=white, minimum width=3.2cm, minimum height=0.8cm, rounded corners] (reb) at (7.5, 0.8) {\small\texttt{action\_reboot\_device}};
...
\begin{figure}[H]
  ...
  \begin{circuitikz}[>=latex]
    % Protocol Receivers
    \draw[dashed, thick, fill=gray!5, rounded corners] (0.0, 2.6) rectangle (5.2, 5.8);
    ...
    % Shared Data Layer
    \draw[dashed, thick, fill=gray!5, rounded corners] (5.9, 0.4) rectangle (11.1, 5.8);
...
\begin{figure}[H]
  ...
  \begin{circuitikz}[>=latex]
    % Writer
    \draw[dashed, thick, fill=gray!5, rounded corners] (0.0, 2.6) rectangle (4.8, 5.2);
    \node[anchor=north, font=\bfseries\small] at (2.4, 5.2) {Network Callback};
    \node[draw, fill=white, minimum width=3.8cm, minimum height=0.7cm, rounded corners, font=\small] (wrt)  at (2.4, 4.5) {Write to Back Buffer};
    \node[draw, fill=white, minimum width=3.8cm, minimum height=0.7cm, rounded corners, font=\small] (swap) at (2.4, 3.3) {Atomic Store (swap)};
    % Buffers
    \draw[dashed, thick, fill=gray!5, rounded corners] (5.6, 0.3) rectangle (10.4, 5.2);
    \node[anchor=north, font=\bfseries\small] at (8.0, 5.2) {Double Buffer};
    \node[draw, fill=white, minimum width=3.8cm, minimum height=0.7cm, rounded corners, font=\small] (b0) at (8.0, 4.5) {Buffer 0 (512 B)};
    \node[draw, fill=white, minimum width=3.8cm, minimum height=0.7cm, rounded corners, font=\small] (b1) at (8.0, 3.3) {Buffer 1 (512 B)};
    ...
    % Reader
    \draw[dashed, thick, fill=gray!5, rounded corners] (11.2, 2.6) rectangle (16.0, 5.2);
    \node[anchor=north, font=\bfseries\small] at (13.6, 5.2) {Output Routine};
    \node[draw, fill=white, minimum width=3.8cm, minimum height=0.7cm, rounded corners, font=\small] (rd)  at (13.6, 4.5) {Atomic Load (index)};
    \node[draw, fill=white, minimum width=3.8cm, minimum height=0.7cm, rounded corners, font=\small] (out) at (13.6, 3.3) {Drive LED String};
    % Arrows
    \draw[-latex, thick] (wrt.south)  -- (swap.north);
    \draw[-latex, thick] (rd.south)   -- (out.north);
    \draw[-latex, thick, rounded corners=4pt] (wrt.east)  -- ++(0.4,0) |- (b0.west);
    \draw[-latex, thick, rounded corners=4pt] (swap.east) -- ++(0.9,0) |- (st.west);
    \draw[-latex, thick, rounded corners=4pt] (st.east)   -- ++(0.9,0) |- (rd.west);
    \draw[-latex, thick, rounded corners=4pt] (b1.east)   -- ++(0.4,0) |- (out.west);
  \end{circuitikz}
...
\begin{figure}[H]
  ...
  \begin{circuitikz}[>=latex]
    ...
    \node[draw, fill=white, minimum width=2.8cm, minimum height=0.8cm, rounded corners] (up)     at (10.5, 1.5) {\texttt{CONNECTED}};
    ...
    void poll() {
        ...
        switch (state_) {
            case wifi_state_t::CONNECTING:
                if (linked) { state_ = wifi_state_t::CONNECTED;
                              event_manager.post(EVT_WIFI_CONNECTED); }
                break;
            case wifi_state_t::CONNECTED:
                if (!linked) { state_        = wifi_state_t::RECONNECTING;
...
private:
    ...
    bool accept_source(const uint8_t* cid, uint8_t priority) {
        ...
        if (!has_source_) {
            ...
            source_.priority = priority; source_.last_packet_ms = now;
    ...
    void check_data_loss() {
        if (has_source_ && (millis() - source_.last_packet_ms > DATA_LOSS_MS)) {
            has_source_ = false;
...
\end{minted}
\caption{\ac{sACN} receiver. \texttt{accept\_source()} implements the single-winner selection policy: it always accepts the known active source and replaces it only when a competing source arrives with strictly higher priority. \texttt{valid\_preamble()} checks the ANSI~E1.31 preamble-size field and the ACN packet identifier, rejecting malformed or non-E1.31 datagrams before any further parsing occurs.}
